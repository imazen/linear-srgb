...
mod tests {
    ...
    #[test]
    fn test_as_slice() {
        let lin_table = LinearTable8::new();
        let slice = lin_table.as_slice();
        assert_eq!(slice.len(), 256);
        assert_eq!(slice[0], 0.0);
        assert!((slice[255] - 1.0).abs() < 1e-6);

        let enc_table = EncodeTable12::new();
        let slice = enc_table.as_slice();
        assert_eq!(slice.len(), 4096);
        assert_eq!(slice[0], 0.0);
        assert!((slice[4095] - 1.0).abs() < 1e-4);
    }

    #[test]
    fn test_default_impls() {
        let lin: LinearTable8 = Default::default();
        assert_eq!(lin.lookup(0), 0.0);

        let enc: EncodeTable12 = Default::default();
        assert_eq!(enc.lookup(0), 0.0);

        let conv: SrgbConverter = Default::default();
        assert_eq!(conv.srgb_u8_to_linear(0), 0.0);
    }

    #[test]
    fn test_converter_linear_to_srgb() {
        let conv = SrgbConverter::new();

        // Boundary values
        assert!((conv.linear_to_srgb(0.0)).abs() < 1e-4);
        assert!((conv.linear_to_srgb(1.0) - 1.0).abs() < 1e-4);

        // Mid-range matches scalar
        for i in 0..=100 {
            let v = i as f32 / 100.0;
            let lut_result = conv.linear_to_srgb(v);
            let scalar_result = crate::scalar::linear_to_srgb(v);
            assert!(
                (lut_result - scalar_result).abs() < 1e-3,
                "linear_to_srgb mismatch at {}: lut={}, scalar={}",
                v,
                lut_result,
                scalar_result
            );
