...
- `default::gamma_to_linear_rgba_slice` / `default::linear_to_gamma_rgba_slice` — alpha-preserving RGBA forms of the custom-gamma slice conversions (straight alpha, no premultiply), closing the gap with the sRGB / BT.709 / PQ / HLG families which all had `_rgba_slice` counterparts. Same clamp + `pow_midp` body and `[v4, v3, neon, wasm128, scalar]` dispatch as the plain `gamma_to_linear_slice` pair; R/G/B are bit-identical to the plain slice, alpha is untouched (and not clamped). The plain `gamma_to_linear_slice` / `linear_to_gamma_slice` docs now warn that they transform alpha too. (#11)
