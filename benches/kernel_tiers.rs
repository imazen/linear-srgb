...
fn bench(suite: &mut Suite) {
    ...
    let f32src: &'static [f32] =
        Box::leak((0..N).map(|i| (i % 1000) as f32 / 1000.0).collect::<Vec<_>>().into_boxed_slice());
    let u8src: &'static [u8] =
        Box::leak((0..N).map(|i| (i % 251) as u8).collect::<Vec<_>>().into_boxed_slice());
    ...
    macro_rules! outplace {
        ($name:expr, $bytes:expr, $mk:expr, $call:expr) => {
            suite.compare($name, |g| {
                ...
                for (arm, simd) in [(TIER_NAME, true), ("scalar", false)] {
                    g.bench(arm, move |b| {
                        b.with_input(move || { set_simd(simd); $mk })
    ...
    inplace!("srgb_to_linear_slice", linear_srgb::default::srgb_to_linear_slice);
    inplace!("linear_to_srgb_slice", linear_srgb::default::linear_to_srgb_slice);
    inplace!("srgb_to_linear_rgba_slice", linear_srgb::default::srgb_to_linear_rgba_slice);
    inplace!("pq_to_linear_slice", linear_srgb::default::pq_to_linear_slice);
    inplace!("hlg_to_linear_slice", linear_srgb::default::hlg_to_linear_slice);
    inplace!("bt709_to_linear_slice", linear_srgb::default::bt709_to_linear_slice);

    outplace!("srgb_u8_to_linear_slice", N, vec![0f32; N],
        |o: &mut Vec<f32>| linear_srgb::default::srgb_u8_to_linear_slice(u8src, o));
    outplace!("linear_to_srgb_u8_slice", N * 4, vec![0u8; N],
        |o: &mut Vec<u8>| linear_srgb::default::linear_to_srgb_u8_slice(f32src, o));
    outplace!("srgb_u8_to_linear_rgba_slice", N, vec![0f32; N],
